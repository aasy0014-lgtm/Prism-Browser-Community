...
export function ProfileDataModal({ open, profile, onClose }: ProfileDataModalProps) {
  ...
  useEffect(() => {
    ...
    else setBackupPasswordOpen(false)
  }, [open, profile?.id])
  ...
  async function exportBackup(password: string): Promise<void> {
    if (!profile) return
    setBackupBusy(true)
    try {
      const result = await window.browserApi.profiles.exportBackup(profile.id, password)
  ...
  return (
    <>
      <Modal open={open} title={`环境数据 · ${profile?.name ?? ''}`} width={680} footer={null} onCancel={onClose} destroyOnHidden>
        ...
        <div className="cookie-transfer">
          <div>
            ...
            <Typography.Text type="secondary">备份包含账号登录数据，将使用密码加密为单个文件；跨系统恢复后部分网站可能需要重新登录。</Typography.Text>
          </div>
          <Button icon={<DownloadOutlined />} loading={backupBusy} disabled={!canClear || backupBusy} onClick={() => setBackupPasswordOpen(true)}>导出加密备份</Button>
        </div>
        <div className="cookie-transfer">
          <div>
      ...
      <ProfileBackupPasswordModal
        mode={backupPasswordOpen ? 'export' : null}
        busy={backupBusy}
        onSubmit={exportBackup}
        onClose={() => setBackupPasswordOpen(false)}
      />
    </>
  )
}
