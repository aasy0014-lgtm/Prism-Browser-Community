...
async function copyStableFile(
  ...
  if (!initial.isFile() || initial.isSymbolicLink() || initial.size !== expectedSize) {
    throw new Error(`备份文件在读取期间发生变化：${archivePath}`)
  }
  const handle = await open(source, READ_ONLY_NOFOLLOW)
  try {
    const opened = await handle.stat()
    if (!opened.isFile() || opened.size !== expectedSize) throw new Error(`备份文件在读取期间发生变化：${archivePath}`)
    let actualSize = 0
    const stream = handle.createReadStream({ autoClose: false })
    for await (const chunk of stream) {
      const data = chunk as Buffer
      actualSize += data.length
      if (actualSize > expectedSize || actualSize > MAX_BACKUP_BYTES) throw new Error(`备份文件在读取期间发生变化：${archivePath}`)
      await onChunk(data)
    }
